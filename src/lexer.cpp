...
std::vector<Token> parseOperations(const std::wstring& seq, size_t line) {
  ...
  auto adUn = [&symbol, &operation, &tokens, &seq = std::as_const(seq),
               &line = std::as_const(line)]() {
    // &, |, +, -

    operation += seq[symbol];

    if (symbol + 1 == seq.size())
      goto exit;

    ++symbol;
    if (seq[symbol] == '=' || seq[symbol] == operation[0] ||
        (operation[0] == '-' && seq[symbol] == '>')) {
      operation += seq[symbol];
    } else
      --symbol;

  exit:
    tokens.push_back({Lexeme::Operation, operation, line});
  };

  auto muEq = [&symbol, &operation, &tokens, &seq = std::as_const(seq),
               &line = std::as_const(line)]() {
    // умножение и прочее
    operation += seq[symbol];
  ...
  auto comparing = [&symbol, &operation, &tokens, &seq = std::as_const(seq),
                    &line = std::as_const(line)]() {
    operation += seq[symbol];
  ...
  while (symbol != seq.size()) {
    if (seq[symbol] == '+' || seq[symbol] == '-' || seq[symbol] == '&' ||
        seq[symbol] == '|')
      adUn();
    else if (seq[symbol] == '*' || seq[symbol] == '/' || seq[symbol] == '!' ||
             seq[symbol] == '=')
      muEq();
    else
      comparing();
...
bool Trie::check_(const wchar_t* s, const Trie::Node* trie, const size_t sz) {
  ...
  if (s[sz] - 'a' < 0 || s[sz] - 'z' > 0 || trie->children[s[sz] - 'a'] == nullptr)
    return false;
  return check_(s, trie->children[s[sz] - 'a'], sz + 1);
}

bool Trie::check_(const std::wstring& s,
